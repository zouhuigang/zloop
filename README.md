...
```
  ▶  就绪      ░░░░░░░░░░░░░░░░ 0%  0/4 条待办 · 跑了 0 轮
  把 demo 服务的冷启动时间从 8 秒降到 1 秒以内

  ▶  下一个   t1 [P0] 找出启动路径上最慢的三处
     验收：有火焰图和三个函数名
  ○  排队中   t2 [P0] 给启动路径加 tracing
  ○  排队中   t3 [P1] 把配置加载改成懒加载
  ○  排队中   t4 [P2] 写压测脚本
...
跑起来之后——`阶段` 一行说清循环走到哪了，页脚是此刻能敲的命令：

```
  🔄 执行中    ████░░░░░░░░░░░░ 25%  1/4 条待办 · 跑了 3 轮 · 花了 $0.84
  把 demo 服务的冷启动时间从 8 秒降到 1 秒以内

  🔄 执行中   t2 [P0] 给启动路径加 tracing
  !  等你回话 t3 [P1] 把配置加载改成懒加载
     ↳ 第 3 步要用付费 SDK，能换成开源的吗？
     答完敲 zloop edit t3 --status open
  ⏳ 等 t3    t4 [P2] 写压测脚本
  … 另有 1 条已完成/已延后 · zloop log 看它们的记录

  阶段    claude 正在做 t2 · 第 3 轮 · 已跑 4m
  后台    runner 在跑（pid 41287）· 日志 .zloop/runner/console.log
...
**② 待办 = 还剩什么。** 每条自带状态词，不用猜符号：

| 标记 | 状态词 | 含义 |
|---|---|---|
| 🔄 | 执行中 | 正被某个会话拿着做 |
| ▶ | 下一个 | 下一轮就做它 |
| ! | 等你回话 | 被 `--block` 了；问题在 `↳`，**解锁命令就在它自己下面** |
| ⏳ | 等 t3 | 在等前置 todo，等谁直接写出来 |
| ○ | 排队中 | 排在后面，没被挡 |

**③ 明细 + 页脚 = 我该敲什么。** 灰标签是情况，青标签是**可以直接抄走的命令**，一行一条：

- `阶段` 和 `后台` **永远在**——任何状态都能答出"现在在哪一步""有没有人在跑"；
- `睡眠` / `文档` / `会话` 只在有话要说时出现（睡眠设置正常、没有缺文档的轮次就不占行）；
- 页脚随状态变：就绪 → `开跑`；执行中 → `写回 zloop done t2 --note … --approach …`；等你决定 → 命令已贴在待办下面；限流中 → `放宽`；已停 → `看失败` + `重跑`；已暂停 → `继续`；完成 → `加活` / `换目标` / `出文档`。
...
```
  ...
  阶段    两轮之间的休息 · 睡到 14:19 醒，还有 4m11s
  后台    没有 runner 在跑
...
```
  ⏳ 等你决定  ████░░░░░░░░░░░░ 25%  1/4 条待办 · 跑了 4 轮
  把 demo 服务的冷启动时间从 8 秒降到 1 秒以内

  !  等你回话 t2 [P0] 给启动路径加 tracing
     ↳ 第 3 步要用付费 SDK，能换成开源的吗？
...
```
  ⏱ 限流中    ░░░░░░░░░░░░░░░░ 0%  0/4 条待办 · 跑了 1 轮
  把 demo 服务的冷启动时间从 8 秒降到 1 秒以内

  ○  排队中   t1 [P0] 找出启动路径上最慢的三处
     验收：有火焰图和三个函数名
  ○  排队中   t2 [P0] 给启动路径加 tracing
  ○  排队中   t3 [P1] 把配置加载改成懒加载
  ○  排队中   t4 [P2] 写压测脚本
