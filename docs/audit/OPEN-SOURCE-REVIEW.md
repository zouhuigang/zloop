...
| `run --git-commit` | Anthropic：每 session 一次 commit | **每轮开工前**给工作树拍快照（路径 → 大小:mtime，`.zloop/` 除外），但只在上一轮"结清"时才重拍；写回后只提交**这条线之后变化的路径**，用 `--pathspec-from-file` 显式点名 commit `zloop <todo>: <note>`；journal 记 `commit`（带 `paths`）/ `commit_held_back`。踩坑：`git add -A -- .` 会把别人的在制品和索引里已暂存的东西一起卷进来（A-12）；快照只拍一次的话，长跑中途邻居新建的文件全算我们的（A-13）；快照重拍太勤会把没写回那轮的产物永远划给别人；pathspec 里显式写被忽略的 `.zloop` 会让 `git add` 退出 1 | `runner_test::git_commit_checkpoints_each_round_excluding_zloop_dir`、`::git_checkpoint_leaves_foreign_work_in_progress_out`、`::git_checkpoint_reclaims_work_left_by_a_round_that_never_wrote_back`、`::git_checkpoint_leaves_out_a_file_a_neighbour_creates_between_rounds` |
