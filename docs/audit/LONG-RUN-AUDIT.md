...
> 目标：**指派一个长程任务后，zloop 能长时间运行而不出问题。** 本文逐项对照 loopx 为"长时运行"设计的机制（见 [`loopx-principles.md` §1 长时运行与上下文管理](../../docs/design/loopx-principles.md#1-长时运行与上下文管理)），核对 zloop 0.2 的现状，给出风险等级与处置。
