...
- **`zloop install --sudoers`**：写 `/etc/sudoers.d/zloop-pmset`，只放行三条精确命令：
  ...
  路径要是别人也占得住名字，装进 `/etc/sudoers.d/` 的就可能不是我们写的那份（[T43](../../docs/audit/FINDINGS.md#3-逐条草稿)，正文见 [CODE-AUDIT §20](../../docs/audit/CODE-AUDIT.md#t43中install_sudoers-的暂存路径别人也能占名装进-etcsudoersd-的可以不是我们写的那份--已修)）。
