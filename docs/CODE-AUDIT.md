...
| [`loopx-scheduling-notes.md` §3.3](loopx-scheduling-notes.md#33-难用的-9-条结构性根因有证据) 的标题写「8 条」，正文列了 **9** 条 | 第 9 条是后来补进去的（`（本项目实测）`那条），标题的计数没跟着改；`DESIGN.md` 引的正是「第 9 条」 | 标题改成 9 条 |
...
以及**链接文字里的号要和锚点的落点一致**（写 `[正文 §20](CODE-AUDIT.md#x)` 就得真落在 `§20` 名下，
