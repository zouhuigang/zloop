...
| `loopx-scheduling-notes.md` §3.3 的标题写「8 条」，正文列了 **9** 条 | 第 9 条是后来补进去的（`（本项目实测）`那条），标题的计数没跟着改；`DESIGN.md` 引的正是「第 9 条」 | 标题改成 9 条 |
...
**正好**是期待的那 9 条——少一条是规则失灵，多一条是规则误伤（假阳性同样会让人把闸关掉）。
