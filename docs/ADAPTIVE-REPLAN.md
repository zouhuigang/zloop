...
> 起因：*"zloop 在创建目标的同时，会有一个监督的 loop 自适应 agent……一开始分析出 5 个 todo，
> 干到第 2 步完成的时候，发现目标有点困难或之前指定的策略行不通，于是基于现状又重新规划出 10 个
> todo，然后重新执行。"*
>
> 上面 §1–§4 那套已经上线了，但它**做不到这件事**。先摆证据，再说要加什么。

## 6. 三个缺口，各有代码为证

### 缺口一：只建议，从不落地

`runner.rs:782` 的重估提示词：

```
**不要**运行任何会改 todo 的命令（plan / edit / done 一律不要），也不要改代码。
```

这是 §「无头模式的红线」有意定的。后果可以量化：2026-08-29 那次 4 小时无头长跑触发了
**5 次重估，改动的 todo 数是 0**。计划从头到尾没变过。

### 缺口二：只认「出岔子」，不认「方向错」

现有五个信号（`replan.rs`）全是**偏离信号**：
...
**没有一个在问「我们离目标更近了吗」。** 用户举的例子恰恰是所有偏离信号都不响的那种：
第 2 步**顺利完成**，但它的结论否定了剩下三条的前提。实测（干净项目，5 条 todo 的目标）：

```
第 1 步顺利完成 → 提示出现 0 次
第 2 步顺利完成，结论是「加缓存这条路走不通，瓶颈在反序列化」 → 提示出现 0 次
此刻 zloop replan → 一个信号都没有
```

而 t3/t4/t5 三条全都建立在「加缓存有效」这个已经被推翻的前提上。**账本上一切正常，计划已经死了。**
...
```
只提**最小改动**……**别重开一张清单**：原计划里还成立的部分要留着（plan repair 优于 full replan）。
```

这条在「策略微调」时是对的（§2 的文献结论），但用户要的 5 条 → 10 条正是 full replan。
两者都需要，缺的是**判断该用哪个**的依据。

## 7. 要加什么：一路新信号 + 一条落地通道 + 一组护栏
...
每条 todo 写回
      ...
      └─ 进展信号（新，问"还到得了吗"）─┘         │
                                                  ├─ 只建议（默认，今天的行为）
                                                  └─ --auto-replan → replan --apply
                                                                          │
                                                                     护栏在代码里强制
```

三件事各自独立可验，所以拆成三条 todo（t2 / t3 / t4）。

## 8. 护栏清单：每条都写清「不加会怎样」

自主改计划是这个项目里第一个**agent 能改自己待办**的能力。护栏必须在**代码里**强制，
不能只写在提示词里——提示词管不住模型，§「无头模式的红线」那条回归测试就是专门演
「不守规矩的模型」的。

| 护栏 | 不加会怎样 |
|---|---|
| **已完成的 todo 和它们的 tick 一条不能动** | 重规划会把历史抹掉，`zloop doc` 和长程审计全部失真；做过的活可能被重新派一遍 |
| **目标文字不可改** | agent 改着改着把目标改成"已经做到的那个"，然后宣布成功。目标是人定的锚 |
| **新清单必须保留所有 done 条目的 id** | id 复用会让 tick 挂到错的 todo 上，账本对不上 |
| **清单规模上限**（比如不超过原来的 3 倍且 ≤ 30 条） | 一次重规划炸出 200 条 todo，跑到天荒地老 |
| **单次运行重规划次数上限** | replan → 新 todo → replan → …… 永不收敛。文献那条"far fewer replans"说的就是这个 |
| **同一批信号只触发一次**（边沿，不是锁存） | 已经踩过：一个 `blocked` 锁存让 5 次重估全由同一个信号触发，占掉全程花费两成多 |
| **每条新 todo 要说清「为什么它通向目标」** | 无法审计。人事后看不出这次改动是不是在绕远路 |
| **改前自动备份 + 一条 tick 记录改了什么** | 改错了没法回退，也说不清是哪一轮改的 |
| **默认关（`--auto-replan` 显式打开）** | 所有现存用法的行为被悄悄改掉；这是行为变更不是 bug 修复 |

## 9. 收敛判据：怎么算"这次重规划是好的"

不是"计划变了"，而是：

1. **每次重规划之后，未完成条数应当趋于减少**——连续两次重规划都让清单变长，就是在发散，该停下问人；
2. **重规划不能重复推翻同一条**——同一个 todo 被删了又加回来，说明模型在原地打转；
3. **上限用完就停下等人**，并且明说"我改了 N 次还没收敛，你来看看"——
   `--block` 已经有这个语义，直接复用。

第 3 条是最后一道闸：**跑飞的时候要停在人面前，而不是安静地跑下去。**

## 10. 落地：三条 todo 各管一段

| | 做了什么 | 关键测试 |
|---|---|---|
| **t2** | `zloop done --rethink` + `kind="rethink"` 信号 | `a_successful_round_can_still_say_the_rest_of_the_plan_is_dead` |
| **t3** | `zloop replan --apply`，六条护栏在代码里强制 | `replan_apply_refuses_rather_than_half_changing_the_plan` |
| **t4** | `zloop run --auto-replan`，两条防跑飞的闸 | `auto_replan_swaps_the_route_mid_run_and_keeps_going` |

### 为什么信号要人主动说（t2）

zloop 判断不了「策略走不通」，所以**不猜**。不做关键词嗅探——那既不可靠又只能认一种语言。
`--rethink` 和 `--pitfall` / `--block` 是同一个形状：**agent 主动交出结构化事实，代码只做机械反应**。
...
信号按**边沿**实现（只认最近一次重估之后新说的）。`blocked` 当年是个锁存，一条挂着的 todo
让一次 4 小时长跑里 5 次重估全由同一个信号触发，占掉两成多花费。

### 六条护栏（t3）

全在 `replan::apply` 一个函数里，违反就**整体拒绝**并指名是哪条——半途改一半的计划比不改更糟。
每条拒绝路径的测试都断言了 `state.json` 一个字节没变。

1. **清单不能空**——重排成 0 条等于悄悄放弃目标
2. **每条都要带 `:: 验收`**——这是本仓库里"为什么这条通向目标"的既有表达；说不出怎么验，
   就是没想清楚它凭什么算一步
3. **`--why` 必填**，记进账本
4. **规模上限：3 倍 + 5，且总数 ≤ 30**。两条同时要——绝对上限单独不够（清单只有 2 条时
   跳到 30 同样失控），相对上限单独也不够（20 → 60 → 180 每步都"只是三倍"）
5. **有轮次在飞就不改**——那个 agent 手上拿的 todo 可能正要被换掉
6. **不动「已完成」和「等你回话」的**——前者动了等于抹历史、长程审计失真；后者身上挂着
   一个**给人的问题**，agent 没资格替人删掉

id 从 `next_id` 往后发、**不复用**：复用会让老 tick 挂到新 todo 上，账本当场对不上。
改前把 `state.json` 备份一份（这是账本里唯一一处批量丢弃用户可见内容的操作）。

### 无头默认不改计划，是代码闸不是提示词（t4）

写"默认关时行为不变"那条测试时，让假宿主**故意抗命**跑了一次 `replan --apply`——**它成功了**。
那条红线一直只由 prompt 守着，可这整个功能的前提就是"提示词管不住模型"，自相矛盾。

改成环境变量闸，和 `hook-stop` 用 `ZLOOP_RUNNER` 是同一个形状：

| 轮次 | 子进程有 `ZLOOP_AUTO_REPLAN` 吗 |
|---|---|
| 干活轮次 | 没有 |
| 回看轮次 | 没有 |
| `preflight` | 没有 |
| 重估轮次，`--auto-replan` **关** | 没有 |
| 重估轮次，`--auto-replan` **开** | **有** |

`replan --apply` 见到 `ZLOOP_RUNNER` 而没有它，直接拒绝。

> **让假宿主演坏人，比断言好人守规矩有用得多。**

### 两条防跑飞的闸（t4）

能改自己计划的循环最容易死在 `replan → 新 todo → replan → ……` 永不收敛上
（§1 文献那条"far fewer replans"说的就是这个）。两条闸，任一触顶就
**停机等人**（`stop reason=replan_diverged`），不是降级继续：
...
第 2 次就停了，没等到 3 次上限，也没跑满 `--max-rounds 30`。

计划到底动没动**不听宿主自称**：改完重读账本比对未完成 todo 的 id，动了才计数、
才记 journal 的 `replan_applied`（带 `open_before` / `open_after` / `nth`）。
